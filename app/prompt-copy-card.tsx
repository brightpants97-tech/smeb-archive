...
위 데이터를 기반으로 아래 7가지를 분석해줘 (기승전결 순서대로):

[기] 이달엔 뭘 했나
1. 콘텐츠 장르 비율 (카테고리별 %)
2. 유튜브+SOOP 통합 키워드 10가지 (각 키워드 관련 영상 한 줄 설명)

[승] 어떻게 됐나
3. 영상 티어표 S~C (조회수·소재·반응성 기준, 이유 한 줄씩 + 1위 인기 이유 포함)

[전] 분석하면
4. 콜라보 케미 랭킹 (등장한 스트리머·유튜버 전원 나열 후 1위 선정)
5. SWOT 분석

[결] 그래서
6. SWOT 기반 생방송 콘텐츠 5가지 추천 (각 추천마다 근거 요소 + 이유)
7. 이번 달 한 문장 결론

📌 출력 형식

● 문체: 실제 신문 기사체 (~했다 / ~으로 분석됐다 / ~인 것으로 나타났다)
● 시청자 애칭은 '케케단', 시청자 언급 시 케케단으로 불러줘
● 과장·추측·판타지 금지 — 제공된 데이터 안에서만 현실적으로 작성
● 데이터에 없는 내용은 추측하지 말고 "확인되지 않았다"로 처리
...
● 1위·최고: 👑 표시
● 티어표·랭킹: 꼴찌(C) → 1위(S) 순 한 줄씩
● 섹션 간 구분선 필수
● 마지막 결론은 큰따옴표 인용구로 마무리
`;
