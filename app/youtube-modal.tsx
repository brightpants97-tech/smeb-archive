...
function VideoModal({ activeId, onClose }: { activeId: string; onClose: () => void }) {
  ...
  return createPortal(
    <div onClick={onClose} style={{ position:'fixed', top:0, left:0, width:'100vw', height:'100vh', zIndex:99999, background:'rgba(0,0,0,0.9)', backdropFilter:'blur(16px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'24px' }}>
      <div onClick={e => e.stopPropagation()} style={{ width:'100%', maxWidth:'960px', position:'relative' }}>
        <button onClick={onClose} style={{ position:'absolute', top:'-48px', right:0, background:'rgba(255,255,255,0.12)', border:'1px solid rgba(255,255,255,0.2)', color:'#fff', borderRadius:'10px', padding:'8px 18px', cursor:'pointer', fontSize:'0.85rem', fontWeight:700 }}>
          ✕ 닫기
        </button>
        <div style={{ aspectRatio:'16/9', borderRadius:'18px', overflow:'hidden' }}>
          <iframe src={`https://www.youtube.com/embed/${activeId}?autoplay=1&rel=0`} width="100%" height="100%" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ border:'none', display:'block', width:'100%', height:'100%' }} />
        </div>
      </div>
    </div>,
    document.body
  ...
  return (
    <div style={{ borderRadius:'16px', overflow:'hidden', border:'1px solid var(--card-border)', background:'var(--card)', display:'flex', flexDirection:'column' }}>
      <div style={{ padding:'10px 14px', borderBottom:'1px solid var(--card-border)', display:'flex', alignItems:'center', gap:'8px' }}>
        <div style={{ width:'8px', height:'8px', borderRadius:'50%', background: isLive ? '#ff4040' : 'var(--text-muted)', boxShadow: isLive ? '0 0 0 3px rgba(255,64,64,0.25)' : 'none', animation: isLive === null ? 'pulse 1.2s ease-in-out infinite' : 'none' }} />
        <span style={{ fontSize:'0.75rem', fontWeight:800, color:'var(--text)', letterSpacing:'0.08em' }}>LIVE NOW</span>
        {isLive && <span style={{ marginLeft:'auto', fontSize:'0.68rem', fontWeight:700, background:'rgba(255,64,64,0.12)', color:'#ff4040', padding:'2px 8px', borderRadius:'100px' }}>방송 중</span>}
      </div>

      <a href={LIVE_URL} target="_blank" rel="noopener noreferrer" style={{ display:'block', position:'relative', textDecoration:'none' }}>
        {isLive && thumbnailUrl ? (
          <img src={thumbnailUrl + '?t=' + Date.now()} alt="라이브 방송" style={{ width:'100%', aspectRatio:'16/9', objectFit:'cover', display:'block' }} />
        ) : isLive === false ? (
          <div style={{ width:'100%', aspectRatio:'16/9', background:'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <img src={PROFILE} alt="스맵" style={{ width:'68px', height:'68px', borderRadius:'50%', border:'3px solid rgba(235,112,26,0.6)', objectFit:'cover' }} />
          </div>
        ) : (
      ...
      <div style={{ padding:'12px 14px', flex:1 }}>
        {isLive ? (
          <a href={LIVE_URL} target="_blank" rel="noopener noreferrer" style={{ display:'block', textAlign:'center', background:'#ff4040', color:'#fff', fontSize:'0.8rem', fontWeight:700, padding:'8px', borderRadius:'10px', textDecoration:'none' }}>
            지금 시청하기 →
          </a>
        ) : (
          <>
            <p style={{ fontSize:'0.75rem', color:'var(--text-muted)', marginBottom:'6px' }}>{isLive === null ? '방송 상태 확인 중...' : '현재 방송 중이 아니에요'}</p>
            {broadStart && isLive === false && (
              <p style={{ fontSize:'0.7rem', color:'var(--text-muted)', marginBottom:'8px' }}>마지막 방송: {new Date(broadStart).toLocaleDateString('ko-KR', { month:'long', day:'numeric' })}</p>
            )}
            <a href={LIVE_URL} target="_blank" rel="noopener noreferrer" style={{ display:'block', textAlign:'center', background:'var(--bg-deeper)', color:'var(--text)', fontSize:'0.75rem', fontWeight:700, padding:'7px', borderRadius:'10px', textDecoration:'none', border:'1px solid var(--card-border)' }}>
              채널 방문하기 →
            </a>
          </>
        )}
  ...
  return (
    <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.4fr 0.9fr 0.9fr', gap:'20px', alignItems:'start' }}>
        {top3[0] && (
          <div onClick={() => setActiveId(top3[0].id)} style={{ cursor:'pointer', borderRadius:'16px', overflow:'hidden', border:'1px solid var(--card-border)', transition:'transform 0.2s, box-shadow 0.2s', background:'var(--card)' }}
            ...
            <div style={{ position:'relative' }}>
              ...
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top, rgba(0,0,0,0.82) 0%, transparent 100%)' }} />
              ...
              <div style={{ position:'absolute', top:'14px', left:'14px', background:'#EB701A', color:'#fff', fontWeight:900, fontSize:'0.85rem', width:'38px', height:'38px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center' }}>1</div>
              <div style={{ position:'absolute', top:'14px', right:'14px', background:'rgba(235,112,26,0.85)', color:'#fff', fontSize:'0.62rem', fontWeight:800, padding:'3px 8px', borderRadius:'4px' }}>BEST</div>
              <div style={{ position:'absolute', bottom:'16px', left:'16px', right:'16px' }}>
                <p style={{ fontWeight:800, fontSize:'1.05rem', lineHeight:1.35, color:'#fff', marginBottom:'10px' }}>{top3[0].title}</p>
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  <span style={{ fontSize:'0.8rem', fontWeight:700, color:'#fff', background:'rgba(235,112,26,0.85)', padding:'3px 10px', borderRadius:'100px' }}>👁 {top3[0].views.toLocaleString()}회</span>
      ...
      <div id="videos" className="sec-light" style={{ margin:'40px calc(-1 * clamp(1.5rem, 5vw, 3rem)) 0', padding:'40px clamp(1.5rem, 5vw, 3rem)' }}>
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:'40px', alignItems:'start' }}>
          <div>
            <p style={{ fontSize:'0.72rem', fontWeight:700, color:'#EB701A', letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:'8px' }}>최신 업로드</p>
          ...
          <div>
            <p style={{ fontSize:'0.72rem', fontWeight:700, color:'#EB701A', letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:'8px' }}>SOOP 공지</p>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {notices.map((n) => (
                <a key={n.id} href={n.url} target="_blank" rel="noopener noreferrer" className="notice-card"
                  style={{ display:'flex', flexDirection:'column', gap:'8px', padding:'16px', background:'var(--card)', borderRadius:'14px', border:'1px solid var(--card-border)', textDecoration:'none', transition:'transform 0.15s' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    <div style={{ display:'flex', gap:'8px', fontSize:'0.72rem', color:'var(--text-muted)' }}>
                      <span>❤️ {n.likes}</span><span>💬 {n.comments}</span>
                    </div>
                  </div>
