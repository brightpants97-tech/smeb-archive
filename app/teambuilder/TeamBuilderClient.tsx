...
function BasicMode() {
  ...
  const [locked, setLocked]   = useState<boolean[]>(POSITIONS.map(()=>false)); // 선수 고정
  ...
  const roll = () => {
    ...
    // 고정된 라인은 현재 result 유지
    const prev = result;
    let tick=0;
    timer.current = setInterval(()=>{
      setResult(POSITIONS.map((pos,i)=>{
        if(locked[i]&&prev) return prev[i]; // 고정된 라인 유지
  ...
  return (
    <>
      ...
      {result && (
        <div style={{ marginTop:'32px', display:'flex', flexDirection:'column', gap:'10px', animation:'popIn 0.3s both' }}>
          {result.map((r,i)=>(
            <div key={r.pos} style={{
              display:'grid', gridTemplateColumns:'1fr 44px 1fr',
              background: locked[i] ? 'rgba(235,112,26,0.08)' : CARD_BG,
              border:`1px solid ${locked[i] ? 'rgba(235,112,26,0.4)' : BORDER}`,
              borderRadius:'14px', overflow:'hidden',
              animation: revealed ? `popIn 0.3s ${i*0.06}s both` : 'none',
              transition:'border-color 0.2s, background 0.2s',
            }}>
              {/* A팀 */}
              <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', gap:'8px' }}>
                <span style={{ fontSize:'0.62rem', fontWeight:800, color:A_COLOR, opacity:0.7 }}>A</span>
                <span style={{ fontWeight:800, fontSize:'1rem', color:'#fff' }}>{rolling ? '···' : r.A}</span>
              </div>
              {/* 포지션 + 고정 버튼 */}
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', borderLeft:`1px solid ${BORDER}`, borderRight:`1px solid ${BORDER}`, background:'rgba(255,255,255,0.015)', gap:'4px' }}>
                <span style={{ fontSize:'0.9rem', lineHeight:1 }}>{r.emoji}</span>
                {!rolling && revealed && (
                  <button
                    ...
                    style={{
                      ...
                      fontSize:'0.75rem', lineHeight:1, padding:'1px',
                      opacity: locked[i] ? 1 : 0.35,
                      filter: locked[i] ? 'none' : 'grayscale(1)',
              ...
              <div style={{ padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'flex-end', gap:'8px' }}>
                <span style={{ fontWeight:800, fontSize:'1rem', color:'#fff' }}>{rolling ? '···' : r.B}</span>
                <span style={{ fontSize:'0.62rem', fontWeight:800, color:B_COLOR, opacity:0.7 }}>B</span>
              </div>
            </div>
          ))}
          {/* 고정 안내 */}
          {revealed && (
            <p style={{ textAlign:'center', fontSize:'0.72rem', color:'rgba(255,255,255,0.25)', margin:'4px 0 0' }}>
              🔒 클릭으로 라인 고정 후 다시 굴리면 고정된 라인은 유지돼요
            </p>
          )}
        </div>
      )}
