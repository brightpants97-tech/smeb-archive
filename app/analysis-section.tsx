...
export default function AnalysisSection({ monthlyTop10, monthTop5, sortedMonths, currentMonth }: Props) {
  const defaultMonth = useMemo(() => {
    ...
    const soopM = new Set(sortedMonths);
    const all = [...new Set([...ytM, ...soopM])].sort().reverse();
    return (ytM.has(currentMonth) || soopM.has(currentMonth)) ? currentMonth : (all[0] || currentMonth);
  }, [monthlyTop10, sortedMonths, currentMonth]);
...
export default function AnalysisSection({ monthlyTop10, monthTop5, sortedMonths, currentMonth }: Props) {
  // 데이터가 있는 달 중 가장 최신 달을 기본값으로
  const defaultMonth = useMemo(() => {
    const ytMonths = new Set(Object.keys(monthlyTop10));
    const soopMonths = new Set(sortedMonths);
    const all = [...new Set([...ytMonths, ...soopMonths])].sort().reverse();
    const hasCurrentData = ytMonths.has(currentMonth) || soopMonths.has(currentMonth);
    return hasCurrentData ? currentMonth : (all[0] || currentMonth);
  }, [monthlyTop10, sortedMonths, currentMonth]);
  const [selectedMonth, setSelectedMonth] = useState(defaultMonth);
  const [result, setResult] = useState<AnalysisResult | null>(null);
  const [loading, setLoading] = useState(false);
