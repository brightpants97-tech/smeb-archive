...
interface Props {
  month: string;
  ytVideos: { title: string; views: number }[];
  soopVods: { title: string; views: number }[];
}

export default function AnalysisSection({ month, ytVideos, soopVods }: Props) {
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({ month, ytVideos, soopVods }),
      });
  ...
  return (
    <div style={{ marginTop: '48px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap' as const, gap: '12px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '6px' }}>
            Gemini AI가 유튜브 TOP10 + SOOP TOP5를 분석합니다
          </p>
        </div>
        {!ran ? (
          <button onClick={run} disabled={loading} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: loading ? 'var(--bg-deeper)' : 'linear-gradient(135deg,#4285f4,#34a853)', color: '#fff', border: 'none', borderRadius: '12px', padding: '12px 24px', fontSize: '0.88rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 16px rgba(66,133,244,0.35)', transition: 'all 0.2s' }}>
            {loading ? (<><span style={{ width: '16px', height: '16px', border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block' }} />분석 중...</>) : (<><span>✨</span>AI 분석 시작</>)}
          </button>
        ) : (
          <button onClick={() => { setRan(false); setResult(null); }} style={{ fontSize: '0.78rem', color: 'var(--text-muted)', background: 'none', border: '1px solid var(--card-border)', borderRadius: '8px', padding: '6px 14px', cursor: 'pointer' }}>다시 분석</button>
        )}
      </div>
      ...
      {!ran && !loading && (
        <div onClick={run} style={{ border: '2px dashed var(--card-border)', borderRadius: '16px', padding: '48px 24px', textAlign: 'center', cursor: 'pointer', transition: 'border-color 0.2s' }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#4285f4'; (e.currentTarget as HTMLElement).style.background = 'rgba(66,133,244,0.04)'; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--card-border)'; (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
          <div style={{ fontSize: '2.5rem', marginBottom: '12px' }}>🤖</div>
          <p style={{ fontSize: '0.95rem', fontWeight: 600, color: 'var(--text)', marginBottom: '6px' }}>클릭해서 {monthLabel} 콘텐츠 분석 시작</p>
          <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>트렌드 · 성과 분석 · 개선 제안을 AI가 정리해 드립니다</p>
        </div>
      )}
      ...
      {result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textAlign: 'center' }}>✨ Gemini 1.5 Flash가 분석 · 데이터 기준: 유튜브 TOP10 + SOOP TOP5</p>
        </div>
      )}
