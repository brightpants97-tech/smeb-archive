...
export default function AnalysisSection({ monthlyTop10, sortedMonths, currentMonth }: Props) {
  const defaultMonth = useMemo(() => {
    const ytM = new Set(Object.keys(monthlyTop10));
    const all = [...ytM].sort().reverse();
    return ytM.has(currentMonth) ? currentMonth : (all[0] || currentMonth);
  }, [monthlyTop10, currentMonth]);
  ...
  const availableMonths = useMemo(() => [...Object.keys(monthlyTop10)].sort().reverse(), [monthlyTop10]);
  ...
  const hasData = ytVideos.length > 0;
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({ month: selectedMonth, ytVideos }),
      });
  ...
  const handleMonthChange = (m: string) => {
    setSelectedMonth(m);
    if (m !== analyzedMonth) { setResult(null); setError(''); }
  };
    ...
    <div style={{ marginTop: '48px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap' as const, gap: '16px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#EB701A', marginBottom: '8px' }}>
            <span style={{ display: 'block', width: '24px', height: '2px', background: '#EB701A', borderRadius: '2px' }} />AI 콘텐츠 분석
          </div>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '6px' }}>Gemini AI가 유튜브 TOP10을 분석합니다</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' as const }}>
          <select value={selectedMonth} onChange={e => handleMonthChange(e.target.value)}
            style={{ padding: '10px 32px 10px 14px', borderRadius: '10px', border: '1px solid var(--card-border)', background: 'var(--card)', color: 'var(--text)', fontSize: '0.88rem', fontWeight: 600, cursor: 'pointer', outline: 'none', appearance: 'none' as const }}>
            {availableMonths.map(m => {
              const [y2, m2] = m.split('-');
              return <option key={m} value={m}>{y2}년 {parseInt(m2)}월 (YT {(monthlyTop10[m]||[]).length})</option>;
            })}
          </select>
          <button onClick={run} disabled={loading || !hasData}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: (!hasData||loading)?'var(--bg-deeper)':'linear-gradient(135deg,#4285f4,#34a853)', color: (!hasData||loading)?'var(--text-muted)':'#fff', border: 'none', borderRadius: '10px', padding: '10px 20px', fontSize: '0.88rem', fontWeight: 700, cursor: (!hasData||loading)?'not-allowed':'pointer', transition: 'all 0.2s' }}>
      ...
      {hasData&&!result&&!loading&&(
        <div onClick={run} style={{border:'2px dashed var(--card-border)',borderRadius:'16px',padding:'48px 24px',textAlign:'center',cursor:'pointer',transition:'all 0.2s'}}
          ...
          <p style={{fontSize:'0.8rem',color:'var(--text-muted)'}}>유튜브 {ytVideos.length}개 데이터 기준</p>
        </div>
      )}
      ...
      {loading&&(<div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        <p style={{textAlign:'center',fontSize:'0.82rem',color:'var(--text-muted)',marginTop:'8px'}}>✨ Gemini가 {monthLabel} 유튜브 TOP10을 분석하고 있습니다...</p>
      </div>)}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 2fr',gap:'16px'}}>
            <div style={{background:'var(--card)',border:'1px solid var(--card-border)',borderRadius:'16px',padding:'24px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'8px'}}>
              <p style={{fontSize:'0.72rem',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase' as const}}>종합 점수</p>
              <div style={{position:'relative',width:'90px',height:'90px'}}>
                <svg viewBox="0 0 90 90" style={{width:'100%',height:'100%',transform:'rotate(-90deg)'}}>
                  ...
                  <circle cx="45" cy="45" r="38" fill="none" stroke={scoreColor(result.overallScore)} strokeWidth="8" strokeDasharray={`${2*Math.PI*38*result.overallScore/100} ${2*Math.PI*38}`} strokeLinecap="round"/>
                </svg>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--card-border)',borderRadius:'16px',padding:'20px'}}>
              <p style={{fontSize:'0.72rem',fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase' as const,marginBottom:'12px'}}>트렌드 키워드</p>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
            <div style={{background:'linear-gradient(135deg,rgba(34,197,94,0.08),rgba(34,197,94,0.02))',border:'1px solid rgba(34,197,94,0.2)',borderRadius:'16px',padding:'20px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'10px'}}><span>🚀</span><span style={{fontSize:'0.72rem',fontWeight:700,color:'#22c55e',textTransform:'uppercase' as const}}>MVP</span></div>
            ...
            <div style={{background:'linear-gradient(135deg,rgba(239,68,68,0.06),rgba(239,68,68,0.01))',border:'1px solid rgba(239,68,68,0.15)',borderRadius:'16px',padding:'20px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'10px'}}><span>💡</span><span style={{fontSize:'0.72rem',fontWeight:700,color:'#ef4444',textTransform:'uppercase' as const}}>개선 여지</span></div>
          ...
          <p style={{fontSize:'0.72rem',color:'var(--text-muted)',textAlign:'center'}}>✨ Gemini 2.0 Flash 분석 · {analyzedMonth} YT TOP{ytVideos.length}</p>
        </div>
      )}
