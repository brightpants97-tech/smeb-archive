...
export default function BanPickClient() {
  ...
  const Pill = ({label,color,bg,bd,onClick}:{label:string;color:string;bg:string;bd:string;onClick?:()=>void}) => (
    <span onClick={onClick} style={{display:'inline-flex',alignItems:'center',padding:'2px 7px',borderRadius:'5px',background:bg,border:`1px solid ${bd}`,color,fontSize:'1rem',fontWeight:700,cursor:onClick?'pointer':'default',whiteSpace:'nowrap' as const}}>{label}</span>
  );
    ...
    <div style={{background:BG,minHeight:'100vh',color:T,fontFamily:'system-ui,sans-serif',paddingBottom:'80px'}}>
      ...
      <div style={{borderBottom:`1px solid ${B}`,padding:'0 clamp(1rem,4vw,2.5rem)',display:'flex',alignItems:'stretch',height:'56px',gap:'0'}}>
        {curTeam ? (
          ...
          <a href="/" style={{color:T2,fontSize:'0.9rem',textDecoration:'none',fontWeight:700,paddingRight:'14px',borderRight:`1px solid ${B}`,display:'flex',alignItems:'center'}}>← 홈</a>
        )}
        <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'0 14px',flex:1}}>
          {curTeam ? (
            <>
              ...
              <span style={{fontWeight:900,fontSize:'0.98rem',color:curTeam.color}}>{curTeam.name}</span>
              <span style={{color:T3,fontSize:'0.9rem'}}>· 선수 {curTeam.players.length}명</span>
            </>
          ) : (
            <span style={{fontWeight:800,fontSize:'0.98rem'}}>🏆 팀 관리</span>
          )}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
          {saved && <span style={{fontSize:'0.94rem',color:'#33CC77',fontWeight:700,animation:'fi 0.2s both'}}>✓ 저장됨</span>}
          {ver && <span style={{fontSize:'0.76rem',color:T3}}>v{ver.slice(0,5)}</span>}
        </div>
      </div>
        ...
        <div style={{padding:'20px clamp(1rem,4vw,2.5rem)',animation:'fi 0.18s both'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
            <div>
              ...
              <div style={{fontSize:'0.86rem',color:T3,marginTop:'3px'}}>팀 카드를 클릭하면 선수 목록을 볼 수 있어요</div>
            </div>
            <button onClick={addTeam} style={{padding:'7px 14px',borderRadius:'8px',border:'none',background:A,color:'#fff',fontSize:'1rem',fontWeight:800,cursor:'pointer'}}>+ 팀 추가</button>
          </div>
                  ...
                  <div key={t.id} className="tc"
                    ...
                    <div style={{padding:'16px 18px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'10px'}}>
                        ...
                        <span style={{marginLeft:'auto',fontSize:'1rem',color:T3,background:'rgba(255,255,255,0.06)',padding:'2px 7px',borderRadius:'100px'}}>{t.players.length}명</span>
                      </div>
                      {/* 선수 라인업 */}
                      <div style={{display:'flex',flexDirection:'column',gap:'4px'}}>
                        {[...t.players].sort((a,b)=>ROLES.indexOf(a.role)-ROLES.indexOf(b.role)).map(p=>(
                          <div key={p.id} style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'0.98rem'}}>
                            <span style={{fontSize:'0.9rem',width:'18px',flexShrink:0}}>{RI[p.role]||'👤'}</span>
                            <span style={{fontWeight:700,color:'rgba(255,255,255,0.8)'}}>{p.name}</span>
                            <span style={{color:T3,fontSize:'1rem'}}>챔피언 {p.champs.length}</span>
                          </div>
                        ))}
                        {t.players.length===0&&<div style={{fontSize:'0.86rem',color:T3}}>선수 없음</div>}
                      </div>
                      {/* 필밴 미리보기 */}
                      {mustbans.length>0&&(
                        <div style={{marginTop:'10px',display:'flex',alignItems:'center',gap:'5px'}}>
                          <span style={{fontSize:'0.98rem',color:'#FF5566',fontWeight:700,flexShrink:0}}>🔴 필밴</span>
                          <div style={{display:'flex',gap:'3px',flexWrap:'wrap'}}>
                            {mustbans.slice(0,7).map(pc=>(
                              <img key={pc.champ.id} src={img(pc.champ)} alt={pc.champ.name} title={pc.champ.name}
                                style={{width:'26px',height:'26px',borderRadius:'5px',objectFit:'cover',border:'1px solid rgba(255,85,102,0.4)'}} />
                            ))}
                            {mustbans.length>7&&<span style={{fontSize:'0.76rem',color:T3,alignSelf:'center'}}>+{mustbans.length-7}</span>}
                          </div>
                        </div>
                      )}
                    </div>
      ...
      {curTeam && (
        <div style={{padding:'16px clamp(1rem,4vw,2.5rem)',animation:'si 0.18s both'}}>
          ...
          {/* 팀 헤더 */}
          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'22px',paddingBottom:'18px',borderBottom:`1px solid ${B}`}}>
            {editT===curTeam.id ? (
              <>
                <input value={curTeam.name} onChange={e=>updTeam(curTeam.id,{name:e.target.value})}
                  style={{background:S,border:`1px solid ${curTeam.color}55`,borderRadius:'7px',padding:'5px 10px',color:T,fontSize:'1rem',fontWeight:900,flex:1,maxWidth:'160px'}} />
              ...
              <span style={{fontWeight:900,fontSize:'1.05rem',color:curTeam.color,flex:1}}>{curTeam.name}</span>
            )}
                ...
                return (
                  <div key={p.id} style={{background:S,border:`1px solid ${isE?curTeam.color+'44':B}`,borderRadius:'12px',overflow:'hidden',transition:'border-color 0.15s'}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'14px 18px'}}>
                      <div style={{width:'40px',height:'40px',borderRadius:'10px',background:`${curTeam.color}18`,border:`1.5px solid ${curTeam.color}33`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1rem',flexShrink:0}}>
                        {RI[p.role]||'👤'}
                      </div>
                      {isE ? (
                        <>
                          <input value={p.name} onChange={e=>updP(curTeam.id,p.id,{name:e.target.value})}
                            style={{background:'rgba(255,255,255,0.07)',border:`1px solid ${curTeam.color}44`,borderRadius:'6px',padding:'4px 8px',color:T,fontSize:'0.98rem',fontWeight:800,width:'100px',flexShrink:0}} />
                          <select value={p.role} onChange={e=>updP(curTeam.id,p.id,{role:e.target.value})}
                            style={{background:'rgba(255,255,255,0.07)',border:`1px solid ${B}`,borderRadius:'6px',padding:'4px 8px',color:T,fontSize:'1rem',cursor:'pointer',flexShrink:0}}>
                            {ROLES.map(r=><option key={r} value={r}>{RI[r]} {r}</option>)}
                          </select>
                        </>
                      ) : (
                        <>
                          <span style={{fontWeight:800,fontSize:'0.98rem',flexShrink:0}}>{p.name}</span>
                    ...
                    {/* 편집 모드: 챔피언 상세 */}
                    {isE && p.champs.length>0 && (
                      <div style={{borderTop:`1px solid ${B}`,padding:'14px 18px',display:'flex',flexDirection:'column',gap:'6px'}}>
                        {p.champs.map(pc => {
                          ...
                          return (
                            <div key={pc.champ.id} style={{display:'flex',flexDirection:'column',gap:'0'}}>
                              <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'6px 10px',background:'rgba(0,0,0,0.02)',borderRadius:`8px ${nk===noteK?'8px 0 0':'8px 8px 8px'}`,border:`1px solid ${tg.bd}`}}>
                                ...
                                <span style={{fontWeight:700,fontSize:'0.94rem',flex:1,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' as const}}>{pc.champ.name}</span>
                                ...
                                <select value={pc.tag} onChange={e=>updPC(curTeam.id,p.id,pc.champ.id,{tag:e.target.value as any})}
                                  style={{background:tg.bg,border:`1px solid ${tg.bd}`,borderRadius:'5px',padding:'3px 6px',color:tg.color,fontSize:'0.94rem',fontWeight:700,cursor:'pointer',flexShrink:0}}>
                                ...
                                <button onClick={()=>delPC(curTeam.id,p.id,pc.champ.id)}
                                  style={{background:'none',border:'none',color:'rgba(255,100,100,0.5)',cursor:'pointer',fontSize:'1rem',padding:'0 2px'}}>✕</button>
                              </div>
                              {noteK===nk&&(
                                <textarea value={pc.note} onChange={e=>updPC(curTeam.id,p.id,pc.champ.id,{note:e.target.value})}
                                  ...
                                  style={{width:'100%',background:'rgba(0,0,0,0.025)',border:`1px solid ${tg.bd}`,borderTop:'none',borderRadius:'0 0 8px 8px',padding:'7px 10px',color:'#333',fontSize:'0.98rem',lineHeight:1.6,resize:'vertical' as const,minHeight:'48px',boxSizing:'border-box' as const}} />
                              )}
                            </div>
                          );
                        })}
      ...
      {picker&&(()=>{
        ...
        return (
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}
            ...
            <div style={{background:'#fff',border:`1px solid ${t.color}33`,borderRadius:'16px',width:'100%',maxWidth:'680px',overflow:'hidden',boxShadow:`0 24px 64px rgba(0,0,0,0.8)`}}>
              {/* 모달 헤더 */}
              <div style={{padding:'12px 16px',borderBottom:`1px solid ${B}`,display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <span style={{fontSize:'0.94rem',color:T2,fontWeight:700}}>· {p.name}</span>
                <span style={{fontSize:'0.86rem',color:T3}}>챔피언 추가</span>
              ...
              <div style={{padding:'10px 14px 8px',display:'flex',gap:'8px',alignItems:'center',borderBottom:`1px solid ${B}`}}>
                <input autoFocus value={ms} onChange={e=>setMs(e.target.value)}
                  placeholder="챔피언 검색..."
                  style={{flex:1,background:S,border:`1px solid ${t.color}44`,borderRadius:'8px',padding:'7px 12px',color:T,fontSize:'0.98rem'}} />
                <span style={{fontSize:'0.94rem',color:T3,flexShrink:0}}>{mf.length}개</span>
              </div>
              {/* 그리드 */}
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(68px,1fr))',gap:'5px'}}>
                  {mf.map(c=>(
                    <div key={c.id} className="ci" onClick={()=>addPC(picker.tid,picker.pid,c)} title={c.name}
                      ...
                      <div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(0,0,0,0.82))',padding:'2px 2px 3px',fontSize:'0.98rem',fontWeight:700,color:T,textAlign:'center',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                        {c.name}
                      </div>
                    </div>
                  ))}
