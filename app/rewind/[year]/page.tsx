...
async function fetchYTVideos(year: number): Promise<Video[]> {
  try {
...
async function fetchSOOPVods(year: number) {
  ...
  const fetchPage = async (page: number) => {
    try {
      const res = await fetch(
        ...
        { headers: { 'Referer': 'https://www.sooplive.com/', 'Origin': 'https://www.sooplive.com', 'User-Agent': 'Mozilla/5.0' }, next: { revalidate: 7200 } }
      );
      const data = await res.json();
...
export default async function RewindPage({ params }: { params: Promise<{ year: string }> }) {
  ...
  const peak       = monthlyData.reduce((b, m) =>
    (m.ytCount + m.soopCount > b.ytCount + b.soopCount ? m : b), monthlyData[0]);
