...
const getYoutubeVideos = unstable_cache(async () => {
  try {
    ...
    const searchRes = await fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${CH}&maxResults=50&order=date&type=video&key=${KEY}`,
      { cache: 'no-store' }
    );
...
const getNotices = unstable_cache(async () => {
  try {
    ...
    const res = await fetch(
      ...
      { headers: { 'Referer': 'https://www.sooplive.com/', 'User-Agent': 'Mozilla/5.0' }, cache: 'no-store' }
    );
    const data = await res.json();
    return (data.contents || []).map((n: any) => ({
      id: n.titleNo, title: n.titleName || '',
      summary: (n.contents || '').replace(/<[^>]+>/g, '').trim().slice(0, 100),
      date: n.regDate?.split(' ')[0] || '',
      likes: n.count?.likeCnt || 0, comments: n.count?.commentCnt || 0,
      url: `https://www.sooplive.com/station/${BJID}/post/${n.titleNo}`,
    }));
  } catch { return []; }
}, ['notices-v2'], { revalidate: 300 });

export default async function Home() {
  const [videos, soopData, notices] = await Promise.all([
    getYoutubeVideos(), getAllVods(), getNotices(),
  ]);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .notice-card { display:flex; align-items:center; gap:16px; padding:16px 20px; background:var(--card); border-radius:14px; border:1px solid var(--card-border); text-decoration:none; transition:transform 0.15s,box-shadow 0.15s; color:inherit; }
